...
public class HBRecorder implements MyListener {
    ...
    public void stoprecording()
    {
        if (service != null){
            ...
            Toast.makeText(context,"screen recording stop  ",Toast.LENGTH_LONG).show();
        }
    }
    ...
    public void pauseScreenRecording(){
        if (service != null){
            ...
            context.startService(service);
            Toast.makeText(context,"screen recording paused ",Toast.LENGTH_LONG).show();
        }
    }
    ...
    public void resumeScreenRecording(){
        if (service != null){
            ...
            context.startService(service);
            Toast.makeText(context,"screen recording resumed ",Toast.LENGTH_LONG).show();
            ...
            service.putExtra(ScreenRecordService.BUNDLED_LISTENER, new ResultReceiver(new Handler()) {
                ...
                protected void onReceiveResult(int resultCode, Bundle resultData) {
                    ...
                    if (resultCode == Activity.RESULT_OK) {
                        ...
                        String onHome_action_perform = resultData.getString("Home_action_perform");
                        String Pause_action_perform = resultData.getString("Pause_action_perform");
                        String Resume_action_perform = resultData.getString("Resume_action_perform");
                        String Stop_action_perform = resultData.getString("Stop_action_perform");
                        ...
                        if (errorListener != null) {
                            ...
                            if (mWasUriSet && !wasOnErrorCalled) {
                                Log.d("nmnmnm","hbrecordercomplete");
                                hbRecorderListener.HBRecorderOnComplete();
                                Log.d("check_intent","hbrecordercomplete");
                              //  hbRecorderListener.backtohome();
                            }
                            wasOnErrorCalled = false;
                        }else if (onStart != null){
                            hbRecorderListener.HBRecorderOnStart();
                        }
                        else if (Stop_action_perform != null){
                            Intent mservice = new Intent(context, ScreenRecordService.class);
                            context.stopService(mservice);
                            hbRecorderListener.backtomyrecording();
                            //stoprecording();

                        }else if (Pause_action_perform != null){
                            pauseScreenRecording();
                        }else if (Resume_action_perform != null){
                            resumeScreenRecording();
                        }
                        else if (onHome_action_perform != null){
                            hbRecorderListener.backtohome();
                           /* Intent mservice = new Intent(context, ScreenRecordService.class);
                            context.stopService(mservice);*/
